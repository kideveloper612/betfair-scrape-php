...
function makeRecord(item, data) {
	var htmlString = '';
	var competition = data['data'][item].league != null ? '<td>'
			+ data['data'][item].league + '</td>' : '<td></td>'
	var isScore = data['data'][item].score != undefined ? '<td>'
			+ data['data'][item].score + '</td>'
			: '<td>0-0</td>';
	htmlString += '<tr rowspan="2">' + competition + '<td colspan="2">'
	...
	htmlString += '<td class="match-name'
			+ status
			+ '"><a href="https://www.betfair.it/exchange/plus/football/market/'
	...
	if (data['data'][item]['MATCH_ODDS'] != null
		...
		var link = 'https://www.betfair.it/exchange/plus/football/market/'
				+ data['data'][item]['MATCH_ODDS'].marketId;
